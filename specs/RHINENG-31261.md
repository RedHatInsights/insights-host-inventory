...
## Root Cause
The tag parsing regex in `Tag.from_string()` (app/utils.py line 288) currently uses `[^=]+` for the value capture group, which allows the '/' character in tag values. This was intentionally changed from `[^=/]+` to `[^=]+` by RHINENG-17554. The same permissive pattern exists in the OpenAPI spec (swagger/api.spec.yaml line 1477). The namespace group already disallows '/' via `[^=/]*`, so no change is needed there. The requirement now is to re-restrict '/' in the value group, effectively reverting the RHINENG-17554 change for the value portion.
