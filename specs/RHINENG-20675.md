...
## Affected Files
- `jobs/cleanup_custom_staleness.py`: This new file needs to be created. It is the main cleanup job that: (1) reads all rows from the Staleness model, (2) checks each row's `conventional_time_to_stale`, `conventional_time_to_stale_warning`, and `conventional_time_to_delete` against the system defaults from `app/culling.py` using a ±3600s tolerance (mirroring the `DEFAULT_STALENESS_EQUIVALENCE_TOLERANCE_SECONDS` constant in `lib/staleness.py`), (3) deletes matching rows via the session, (4) invalidates the Redis cache for each deleted org_id via `StalenessCache.delete()`, and (5) supports `DRY_RUN` and `SUSPEND_JOB` safety-gate env vars following the same pattern as `jobs/update_staleness.py` and `jobs/delete_empty_org_groups.py`.
...
### Step 4: Create a test file modelled after `tests/test_job_update_staleness.py` and `tests/test_job_delete_empty_org_groups.py`. Tests should cover: (1) rows with all three conventional fields within tolerance (<3600s of defaults) are deleted; (2) rows with at least one field at exactly 3600s from the default are retained (boundary test); (3) rows with at least one field more than 3600s from the default are retained; (4) `DRY_RUN=true` logs intended deletes but makes no DB writes; (5) `DRY_RUN` env var absent defaults to `true` and makes no writes; (6) `StalenessCache.delete` is called exactly once per deleted org_id and not called for retained rows; (7) a no-op run when the staleness table is empty completes without error.
...
- If a Staleness row has a NULL value for any conventional field (unlikely given the model defaults, but possible from legacy data), the abs() comparison would fail — the job should treat NULL as not-equivalent-to-defaults (i.e., skip/retain the row) and log a warning.
...
## Constraints
- The tolerance check must be strictly less than 3600 seconds (i.e., `abs(value - default) < 3600`), consistent with the existing `DEFAULT_STALENESS_EQUIVALENCE_TOLERANCE_SECONDS` in `lib/staleness.py` — a difference of exactly 3600 seconds must NOT be treated as equivalent.
