...
- `jobs/clean_custom_staleness.py` (create): Create the new job script. It should define a `THRESHOLD = 3600` constant, import the three system-default constants from `app.culling`, and define a helper function `_is_within_threshold(row, threshold)` that returns `True` when all three staleness fields on a given `Staleness` row are within `threshold` seconds of their respective defaults (using `abs(field - default) <= threshold`). This helper keeps the comparison logic in one place and simplifies both the job and its tests. The job should push the threshold filtering into the DB query where practical (e.g., using SQL `BETWEEN` / `abs()` expressions) so that only candidate rows are loaded, rather than fetching the entire table into memory. If that is not straightforward with the current ORM model, an acceptable alternative is to query in batches (e.g., `yield_per` / server-side cursor) and apply the helper in Python. Log what will be deleted, and — when `DRY_RUN` is false — delete those rows via `session_guard` and call `StalenessCache.delete(org_id)` for each, handling cache failures gracefully. Must support `SUSPEND_JOB` and `DRY_RUN` env vars defaulting to `true`, following the structure of `jobs/update_staleness.py`.
...
- Prefer pushing the threshold filter into the DB query (SQL-level filtering) so only candidate rows are transferred. If that proves impractical with the ORM, use `yield_per()` / server-side cursors to avoid loading the entire table into memory. For a one-time cleanup the risk is low, but DB-side filtering is the better default.
...
- The per-field threshold comparison logic should be encapsulated in a single helper (`_is_within_threshold`) to avoid duplication across the job implementation and its tests.
