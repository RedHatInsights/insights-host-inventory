...
- `.pre-commit-config.yaml` (modify): Add a new hook entry for `check-shebang-scripts-are-executable` to the existing `pre-commit/pre-commit-hooks` block (rev v6.0.0), unscoped so it applies to all shebang scripts in the repository. Place it immediately after the existing `debug-statements` hook entry, following the same YAML style as the other hook entries in that block.
