...
@pytest.mark.parametrize("ip", ["192.168.1.1",
...
@pytest.mark.parametrize("ip", ["19216811", "", "K00L:0:0:0:0:0:0:1", ])
def test_invalid_ip_address(ip):
    assert not verify_ip_address_format(ip)


@pytest.mark.parametrize("mac", ["aa:bb:cc:dd:ee:ff",
                                 "AA:22:CC:DD:EE:FF",
                                 None, ])
...
@pytest.mark.parametrize("mac", ["aa:bb:cc:dd:ee:xx", "19216811", "", ])
