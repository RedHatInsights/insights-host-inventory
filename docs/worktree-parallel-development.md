...
| Python venv (main) | Pipenv creates venvs by project directory path — each worktree gets its own |
| Python venv (IQE) | Same as above, via `setup-iqe.sh` |
...
cd worktrees/my-feature && unset PIPENV_PIPFILE && pipenv shell
...
Destroying a worktree cleans up everything: containers, Podman network, DB data directory, both pipenv venvs, the git worktree, and the branch (if fully merged).
