...
flowchart TD
    ...
    HC4 --> HC5[✓ Host Created<br/>No Group]

    %% Group Creation Flow
    GC1 --> GC3{Check RBAC v1<br/>Create Permission}
    ...
    HG4 -->|Exists| HG5[Add host to group]
    HG5 --> HG6[Add Host-Group Association]
...
- **Optional Group Membership**: Hosts can exist without being in a group (with `[]` in groups column)
- **Synchronous Operations**: Direct DB updates are made when MQ Events are consumed to create/update hosts and MQ events are produced for every change to hosts.
...
flowchart TD
    ...
    KC1 --> KC3[Create Host in HBI DB<br/>groups = blank initially]
    KC3 --> KC4[Get Ungrouped Hosts workspace_id using RBAC v2 API]
    KC4 --> KC5{Does Ungrouped Hosts<br/>Workspace Exist?}
    KC5 -->|No| KC6[⚡ HBI creates new Workspace named <br/>Ungrouped Hosts]
    KC5 -->|Yes| KC7[Get workspace_id from RBAC v2]
    KC6 --> KC8[⏳ HBI waits for workspace_id from RBAC v2]
    ...
    %% Group Creation Flow
    KGC1 --> KGC3{Check RBAC v1<br/>Create Permission}
    KGC3 -->|Denied| KGC4[✗ Return Error]
    KGC3 -->|Granted| KGC5{Check RBAC v2 DB<br/>Does Workspace Exist?}
    ...
    KHG2 -->|Granted| KHG4{Check RBAC v2 DB<br/>Does Workspace Exist?}
    ...
    KRG2 -->|Granted| KRG3{Check RBAC v2 DB<br/>Does Workspace Exist?}
