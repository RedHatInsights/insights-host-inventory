...
class HBIMessageConsumerBase:
    ...
    def _process_batch(self) -> None:
        ...
        """
        # Consume messages outside the trace span so that idle polls
        # (which return no messages) don't create empty traces in Tempo.
        messages = self.consumer.consume(
            num_messages=inventory_config().mq_db_batch_max_messages,
            timeout=inventory_config().mq_db_batch_max_seconds,
        )

        # Filter out None and error messages before creating a trace span.
        valid_messages = []
        for msg in messages:
            if msg is None:
                continue
        ...
        # The mq.batch span wraps session_guard so that the DB flush/commit
        # (triggered when session_guard exits) appear as child spans rather
        # than orphan root traces.
