...
def add_host(host_data):
    ...
    with PayloadTrackerProcessingContext(
        ...
        try:
            ...
            (output_host, insights_id, add_result) = host_repository.add_host(
            ...
            return (output_host, insights_id, add_result)
...
def handle_message(message, event_producer):
    ...
    with PayloadTrackerContext(payload_tracker, received_status_message="message received"):
        (output_host, insights_id, add_results) = add_host(validated_operation_msg["data"])
        ...
        event_producer.write_event(event, output_host["id"], message_headers(add_results, insights_id), Topic.egress)

        # for transition to platform.inventory.events
        if inventory_config().secondary_topic_enabled:
            event_producer.write_event(
                event, output_host["id"], message_headers(add_results, insights_id), Topic.events
            )
