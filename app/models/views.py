...
class InventoryView(db.Model):
    ...
    name = db.Column(db.String(MAX_VIEW_NAME_LENGTH), nullable=False)
...
class UserViewPreference(db.Model):
    ...
    __table_args__ = (
        Index("idx_uvp_default_view_id", "default_view_id"),
        {"schema": INVENTORY_SCHEMA},
    )

    org_id = db.Column(db.String(36), primary_key=True)
    user_id = db.Column(db.String(255), primary_key=True)
    default_view_id = db.Column(
        UUID(as_uuid=True),
        ForeignKey(f"{INVENTORY_SCHEMA}.inventory_views.id", ondelete="CASCADE"),
        nullable=False,
    )
    updated_on = db.Column(db.DateTime(timezone=True), default=_time_now, onupdate=_time_now, nullable=False)
