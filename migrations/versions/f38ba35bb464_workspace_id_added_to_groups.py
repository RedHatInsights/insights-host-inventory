...
def upgrade():
    ...
    with op.batch_alter_table('hosts', schema='hbi') as batch_op:
        batch_op.create_index('hosts_subscription_manager_id_index', [sa.text("(canonical_facts ->> 'subscription_manager_id')")], unique=False)
        ...
        batch_op.create_index('idxsystem_profile_facts', ['system_profile_facts'], unique=False, postgresql_using='gin')
    ...
    op.create_table('ungrouped_workspaces',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('org_id', sa.String(length=36), nullable=False),
    sa.Column('name', sa.String(length=255), nullable=False),
    ...
    sa.Column('created_on', sa.DateTime(timezone=True), nullable=True),
    sa.Column('modified_on', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['group_id'], ['hbi.groups.id'], ),
    sa.PrimaryKeyConstraint('id'),
    schema='hbi'
    )
    ...
    sa.ForeignKeyConstraint(['group_id'], ['hbi.groups.id'], ),
    sa.ForeignKeyConstraint(['host_id'], ['hbi.hosts.id'], ),
