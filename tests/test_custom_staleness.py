...
from api.staleness import _update_hosts_staleness_async
...
def test_async_host_update_create_custom_staleness(
    db_create_staleness_culling, db_get_hosts, db_create_multiple_hosts, api_get, flask_app
):
    with (
        ...
        host_update_thread = Thread(target=_update_hosts_staleness_async, daemon=True, args=("test", flask_app.app))
        host_update_thread.start()
        host_update_thread.join()
...
def test_async_host_update_delete_custom_staleness(
    db_create_staleness_culling,
    db_delete_staleness_culling,
    db_get_hosts,
    db_create_multiple_hosts,
    api_get,
    flask_app,
    ...
    with (
        ...
        with flask_app.app.app_context():
            ...
            db_delete_staleness_culling(hosts_before_update[0].org_id)
