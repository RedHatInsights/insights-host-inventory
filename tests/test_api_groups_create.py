...
UNFILTERED_GROUPS_WRITE_FOR_CREATE_MSG = (
    "Unfiltered inventory:groups:write RBAC permission is required in order to create new groups."
)


@pytest.mark.usefixtures("enable_rbac")
def test_create_group_filtered_rbac_write_forbidden_when_kessel_bypassed(
    mocker: MockerFixture,
    api_create_group,
    inventory_config: Config,
) -> None:
    """Filtered groups:write with attributeFilter is rejected when Kessel is bypassed.

    Same RBAC mock as the Kessel-enabled success test; only ``bypass_kessel`` differs.
    The legacy DB create path requires an unfiltered inventory:groups:write permission.
    """
    inventory_config.bypass_kessel = True
    ...
    group_data = {"name": "filtered_rbac_bypass_kessel_group", "host_ids": []}
    response_status, response_data = api_create_group(group_data)

    assert_response_status(response_status, 403)
    assert response_data.get("detail") == UNFILTERED_GROUPS_WRITE_FOR_CREATE_MSG
...
def test_create_group_filtered_rbac_write_succeeds_when_kessel_not_bypassed(
    ...
    body = response.json()
    assert UNFILTERED_GROUPS_WRITE_FOR_CREATE_MSG not in (body.get("detail") or "")
