...
def test_xjoin_search_query_to_delete_all_hosts(mocker, graphql_query_empty_response, api_delete_all_hosts):
    query_params = {"delete_all": True, "confirm_delete_all": True}
    api_delete_all_hosts(query_params)

    # using mocker.ANY because no filter is used to fetch all hosts.
    graphql_query_empty_response.assert_called_once_with(
        HOST_IDS_QUERY, {"filter": (), "limit": mocker.ANY}, mocker.ANY
    )
