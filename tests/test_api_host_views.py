...
class TestHostViewAppDataEdgeCases:
    ...
    def test_image_builder_data(self, api_get, db_create_host, db_create_host_app_data):
        """Image Builder data should include image_name and image_status when requested."""
        host = db_create_host()
        host_id = str(host.id)
        host_org_id = host.org_id
...
class TestHostViewSparseFieldsets:
    ...
    def test_request_multiple_apps(self, api_get, db_create_host, db_create_host_app_data):
        ...
        # Request advisor and vulnerability, but not patch
        url = build_host_view_url(query="?fields[advisor]=&fields[vulnerability]=")
        response_status, response_data = api_get(url)
    ...
    def test_request_app_with_no_data_in_db(self, api_get, db_create_host, db_create_host_app_data):
        ...
        # Request vulnerability which has no data
        url = build_host_view_url(query="?fields[vulnerability]=")
        response_status, response_data = api_get(url)
