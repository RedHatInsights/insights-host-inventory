...
def test_literal_asterisk_in_data(db_create_host, api_get):
    """Test that hosts with literal asterisks in their data can be found."""
    ...
    # Test that wildcard query matches both
    url = build_hosts_url(query="?filter[system_profile][insights_client_version]=3.0.*")
    response_status, response_data = api_get(url)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("3.0.*", "3.0.*", True),  # Literal asterisk matches wildcard
        ("3.0.*", "*", True),  # Literal asterisk matches broader wildcard
    ],
)
...
def test_wildcard_escaping_functionality(db_create_host, api_get):
    """Test that the wildcard escaping function works correctly."""
    # This test verifies that the _escape_wildcard_value function is being called
    # and that basic wildcard functionality works as expected
...
def test_multiple_wildcard_fields_support(db_create_host, api_get):
    """Test wildcard functionality across different system profile fields that support it."""
    # Create a host with data in wildcard-enabled fields
    host_data = {
        "insights_client_version": "3.0.1",
        # Note: We only test insights_client_version as it's confirmed to support wildcards
        # Other fields may or may not support wildcards depending on their spec definition
    }
    ...
    response_status, response_data = api_get(url)
    assert response_status == 200
    assert host_id in [r["id"] for r in response_data["results"]]


def test_wildcard_with_special_characters(db_create_host, api_get):
    """Test wildcard functionality with various special characters."""
    # Create hosts with versions containing special characters
    db_create_host(extra_data={"system_profile_facts": {"insights_client_version": "3.0.1-el7"}})
    db_create_host(extra_data={"system_profile_facts": {"insights_client_version": "3.0.2-el8"}})
    db_create_host(extra_data={"system_profile_facts": {"insights_client_version": "3.0.1-fc30"}})

    # Test wildcard with dash
    url1 = build_hosts_url(query="?filter[system_profile][insights_client_version]=3.0.*-el*")
    response_status1, response_data1 = api_get(url1)
    assert response_status1 == 200
    assert response_data1["count"] == 2  # Should match el7 and el8 versions

    # Test exact match with special characters
    url2 = build_hosts_url(query="?filter[system_profile][insights_client_version]=3.0.1-fc30")
    response_status2, response_data2 = api_get(url2)
    assert response_status2 == 200
    assert response_data2["count"] == 1  # Should match exactly fc30 version
