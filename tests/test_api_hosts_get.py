...
# Tests for URL-encoded asterisk handling in wildcard fields (RHINENG-4809)
@pytest.mark.parametrize(
    "filter_value,expected_matches",
    [
...
def test_wildcard_url_encoded_asterisk_handling(db_create_host, api_get, filter_value, expected_matches):
    ...
    actual_values = []
    for result in response_data["results"]:
        # Find the corresponding value for this host ID
...
def test_wildcard_backslash_escaping_compatibility(db_create_host, api_get):
...
def test_wildcard_complex_mixed_scenarios(db_create_host, api_get):
...
def test_wildcard_backward_compatibility(db_create_host, api_get):
    """Test that existing wildcard functionality continues to work unchanged."""
    # Create test hosts with various insights_client_version values
    test_values = [
        "3.0.1-2.el4_2",
        "3.0.5-1.el7_1",
