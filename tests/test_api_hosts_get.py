...
@pytest.mark.parametrize(
    "test_case,host_data,filter_field,filter_query,expected_matches",
    [
        # Test escaped asterisk literal match
        (
            "escaped_asterisk_literal",
            [
                {"insights_client_version": "3.0.*-2.el8"},  # literal asterisk
                {"insights_client_version": "3.0.1-2.el8"},  # would match wildcard
                {"insights_client_version": "4.0.1-2.el8"},  # no match
            ],
            "insights_client_version",
            "3.0.\\*-2.el8",
            [0],  # only first host should match
        ),
        # Test regular asterisk wildcard
        (
            "regular_asterisk_wildcard",
            [
                {"insights_client_version": "3.0.1-2.el8"},
                {"insights_client_version": "3.0.5-1.el8"},
                {"insights_client_version": "4.0.1-2.el8"},  # no match
            ],
            "insights_client_version",
            "3.0.*",
            [0, 1],  # first two hosts should match
        ),
        # Test URL-encoded escaped asterisk
        (
            "url_encoded_escaped_asterisk",
            [
                {"insights_client_version": "3.0.*-2.el8"},  # literal asterisk
                {"insights_client_version": "3.0.1-2.el8"},  # would match wildcard
                {"insights_client_version": "4.0.1-2.el8"},  # no match
            ...
            "insights_client_version",
            "3.0.%5C%2A-2.el8",  # URL-encoded \*
            [0],  # only first host should match
        ),
        # Test mixed escaped and wildcard asterisks
        (
            "mixed_escaped_and_wildcard",
            [
                {"os_release": "8.5*server_v1.2"},  # exact match
                {"os_release": "8.5*server_v2.5"},  # wildcard match
                {"os_release": "8.5_server_v1.2"},  # no match (no literal *)
            ],
            "os_release",
            r"8.5\*server_v*",
            [0, 1],  # both hosts with literal * should match
        ),
        # Test multiple escaped asterisks
        (
            "multiple_escaped_asterisks",
            [
                {"insights_client_version": "3.0.*-2.el8.*"},  # literal asterisks
                {"insights_client_version": "3.0.1-2.el8.5"},  # would match wildcards
                {"insights_client_version": "3.0.x-2.el8.x"},  # no match
            ],
            "insights_client_version",
            "3.0.\\*-2.el8.\\*",
            [0],  # only first host should match
        ),
        # Test with other wildcard field (bootc_status.booted.image)
        (
            "bootc_image_escaped_asterisk",
            [
                {"bootc_status": {"booted": {"image": "quay.io/repo*:latest"}}},  # literal asterisk
                {"bootc_status": {"booted": {"image": "quay.io/repo_test:latest"}}},  # no match
            ],
            "bootc_status][booted][image",
            "quay.io/repo\\*:latest",
            [0],  # only first host should match
        ),
        ...
        (
            ...
            [
                ...
                {"insights_client_version": "3.0.5-1.el4_2"},
                {"insights_client_version": "4.0.1-2.el8_5"},  # no match
            ],
            "insights_client_version",
            "3.0.*",
            [0, 1],  # first two hosts should match
...
def test_escaped_asterisk_filtering(
    db_create_host, api_get, test_case, host_data, filter_field, filter_query, expected_matches
):
    ...
    url = build_hosts_url(query=f"?filter[system_profile][{filter_field}]={filter_query}")
    response_status, response_data = api_get(url)
    ...
    for i, host in enumerate(hosts):
        if i in expected_matches:
            assert str(host.id) in response_ids, f"Test case '{test_case}': Host {i} should match but doesn't"
        else:
            assert str(host.id) not in response_ids, f"Test case '{test_case}': Host {i} should not match but does"
