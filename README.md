...
Use the environment variable `SYS_PROFILE_FIELDS_LOG` to log the System Profile fields of a host.
These fields are logged when `adding`, `updating` or `deleting` a host from inventory. It is very helpfull to debug hosts in Kibana.

Below is an example on how to use to use the environment variable:

```bash
SYS_PROFILE_FIELDS_LOG = "cpu_model,disk_devices"
```
