...
class TagsAPIWrapper(BaseEntity):
    ...
    def get_tags_json(self, **kwargs: Any) -> dict[str, Any]:
        """GET /tags, return parsed JSON body. Accepts same kwargs as get_tags_response."""
        return self.get_tags_response(**kwargs).json()

    def get_tags(self, **kwargs: Any) -> list[dict[str, Any]]:
        """GET /tags, return just the ``results`` list."""
        return self.get_tags_json(**kwargs)["results"]
