...
class HostsAPIWrapper(BaseEntity):
    ...
    def delete_by_id(
        ...
        **api_kwargs: Any,
        ...
        while host_ids:
            try:
                self.delete_by_id_raw(host_ids[:100], **api_kwargs)
            except ApiException as err:
                if err.status == 404:
                    logger.info(
                        f"Couldn't delete hosts {host_ids}, because they were not found in HBI."
                    )
                elif err.status == 403:
                    logger.info(
                        f"Couldn't delete hosts {host_ids}, because they were not found in Kessel,"
                ...
                if wait_for_deleted:
                    self.wait_for_deleted(host_ids[:100], delay=delay, retries=retries)
                host_ids = host_ids[100:]
