...
class TestGetHosts:
    ...
    def test_branch_id_parameter(
        ...
        # GET with insights_id parameter in the URL and branch_id
        response = host_inventory.apis.hosts.get_hosts_by_id_response(
            insights_id, branch_id="some_fake_branch"
        )
        assert not response.total

        # GET with insights_id parameter in the URL, no branch_id
        response = host_inventory.apis.hosts.get_hosts_by_id_response(insights_id)
        assert not response.total

        # GET with host's id in the URL
        response = host_inventory.apis.hosts.get_hosts_by_id_response(
            host_id, branch_id="some_branch"
        )
        ...
        second_host_id = host_2.id
        assert second_host_id != host_id

        two_hosts = [host_id, second_host_id]
