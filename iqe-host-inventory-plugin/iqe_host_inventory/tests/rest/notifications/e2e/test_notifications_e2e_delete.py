...
def setup_delete_notifications(host_inventory: ApplicationHostInventory) -> Generator[None]:
    """
    Creates Behavior Group (BG) in Notifications service.
    """

    email_alert = NotificationsEmailAlert(
        host_inventory, event_name="System deleted", bundle_name="rhel"
    )
...
def check_instant_email(
    host_inventory: ApplicationHostInventory, display_name: str, base_url: str
) -> None:
    """
...
def test_notifications_e2e_delete_by_id(
    ...
    check_instant_email(app_host_inventory, host.display_name, hbi_base_url)
    # Notifications APIs don't work with cert auth
    check_event_log_delete(host_inventory, host, tags, sp)
...
def test_notifications_e2e_delete_filtered(
    ...
    check_instant_email(host_inventory, host.display_name, hbi_base_url)
    check_event_log_delete(host_inventory, host, tags, sp)
