...
@pytest.mark.usefixtures("rbac_inventory_groups_read_granular_user_setup_class")
class TestRBACGranularGroupsReadPermission:
    def test_rbac_granular_groups_read_permission_get_groups_list(
        ...
        """
        expected_groups = rbac_setup_resources.groups[:2]
...
class TestRBACGranularGroupsWritePermission:
    ...
    def test_rbac_granular_groups_write_permission_patch_group(
        ...
        def teardown() -> None:
            for g in correct_groups:
                host_inventory.apis.groups.patch_group(g, name=g.name)
    ...
    def test_rbac_granular_groups_write_permission_remove_hosts(
        ...
        """
        groups = rbac_setup_resources.groups[:2]
    ...
    def test_rbac_granular_groups_write_permission_patch_group_wrong(
        ...
        """
        group = rbac_setup_resources.groups[2]
    ...
    def test_rbac_granular_groups_write_permission_remove_hosts_wrong(
        ...
        """
        group = rbac_setup_resources.groups[2]
        hosts = rbac_setup_resources.hosts[2]
    ...
    def test_rbac_granular_groups_write_permission_remove_hosts_from_multiple_groups_good_and_bad(
        ...
        metadata:
          ...
          title: Test that users with granular RBAC access can't remove hosts from groups via
                 DELETE /groups/hosts/<host_ids> if the request includes good and bad hosts
        """
        groups = rbac_setup_resources.groups[:-1]
        hosts_by_group = rbac_setup_resources.hosts[: len(groups)]
    ...
    def test_rbac_granular_groups_write_permission_add_hosts_wrong(
        ...
        """
        group = rbac_setup_resources.groups[2]
        original_hosts = rbac_setup_resources.hosts[2]
        new_hosts = rbac_setup_resources.hosts[3]
...
class TestRBACGranularGroupsWrongPermissionWriteEndpoints:
    ...
    def test_rbac_granular_groups_wrong_permission_add_hosts(
        ...
        original_hosts = rbac_setup_resources.hosts[1]
        new_host = rbac_setup_resources.hosts[3][0]
...
@pytest.mark.usefixtures("rbac_inventory_groups_write_granular_user_setup_class")
...
def setup_permissions_with_and_without_resource_definitions(
    ...
    """
    hbi_groups = [rbac_setup_resources.groups[0]]
...
@pytest.mark.usefixtures("setup_permissions_with_and_without_resource_definitions")
...
@pytest.mark.usefixtures("setup_resource_definitions_in_multiple_roles")
...
@pytest.mark.usefixtures("setup_rbac_bad_key")
