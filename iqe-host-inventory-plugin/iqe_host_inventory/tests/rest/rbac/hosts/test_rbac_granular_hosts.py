...
def test_rbac_granular_hosts_read_wrong_group_id(
    rbac_setup_resources_for_granular_rbac: RBacResources,
    hbi_non_org_admin_user_rbac_setup,
    host_inventory_non_org_admin,
):
    ...
    metadata:
        ...
        importance: high
        negative: true
        title: Test that users with granular RBAC not existing group can't access hosts
    """
    # Setup
    hbi_non_org_admin_user_rbac_setup(
        permissions=[RBACInventoryPermission.HOSTS_READ], hbi_groups=[generate_uuid()]
    )
...
class TestRBACGranularHostsDifferentAccount:
    def test_rbac_granular_hosts_read_permission_different_account(
        self,
        hbi_non_org_admin_user_rbac_setup,
        host_inventory_secondary: ApplicationHostInventory,
        host_inventory_non_org_admin: ApplicationHostInventory,
        hbi_secondary_upload_prepare_host_class: HostOut,
    ):
        """
        https://issues.redhat.com/browse/ESSNTL-4961
        ...
        metadata:
          ...
          importance: high
          negative: true
          title: Test that users with granular RBAC group from different account can't access hosts
        """
        # Setup
        group = host_inventory_secondary.apis.groups.create_group(
            generate_display_name(), hosts=hbi_secondary_upload_prepare_host_class
        )
        hbi_non_org_admin_user_rbac_setup(
            permissions=[RBACInventoryPermission.HOSTS_READ], hbi_groups=[group]
        )

        # Test
        response = host_inventory_non_org_admin.apis.hosts.get_hosts_by_id_response(
            hbi_secondary_upload_prepare_host_class
        )
        assert response.count == 0
        assert response.total == 0
        assert len(response.results) == 0

    def test_rbac_granular_hosts_write_permission_different_account(
        self,
        hbi_non_org_admin_user_rbac_setup,
        host_inventory_secondary: ApplicationHostInventory,
        host_inventory_non_org_admin: ApplicationHostInventory,
        ...
        metadata:
          ...
          importance: high
          negative: true
          title: Test that users with granular RBAC group from different account can't edit hosts
        """
        # Setup
        group = host_inventory_secondary.apis.groups.create_group(
            generate_display_name(), hosts=hbi_secondary_upload_prepare_host_class
        ...
        hbi_non_org_admin_user_rbac_setup(
            permissions=[RBACInventoryPermission.HOSTS_WRITE], hbi_groups=[group]
        )
        ...
        host_inventory_secondary.apis.hosts.verify_not_updated(
            hbi_secondary_upload_prepare_host_class,
            display_name=hbi_secondary_upload_prepare_host_class.display_name,
        )
