...
def _count_affected_rows(session: Session, table: str) -> int:
    """
    Count rows that need to be updated in the specified table.

    Args:
        session: Database session
        table: Table name ('hosts' or 'system_profiles_dynamic')
    ...
    if table == "hosts":
        count_sql = f"""
            ...
            WHERE system_profile_facts -> 'workloads' IS NOT NULL
                AND EXISTS (
                    SELECT 1
                    FROM jsonb_each(system_profile_facts -> 'workloads') AS wl(key, value),
                         jsonb_each(value) AS fields(field_key, field_value)
                    WHERE field_value = 'null'::jsonb
                )
        """
    else:  # system_profiles_dynamic
        count_sql = f"""
            SELECT COUNT(*)
...
def _update_hosts_batch(session: Session, batch_size: int) -> int:
    """
    Update a batch of hosts with JSON null values in workloads.
    ...
    sql_update_hosts_batch = f"""
        ...
        UPDATE {INVENTORY_SCHEMA}.hosts h
        SET system_profile_facts = jsonb_set(
            system_profile_facts,
            '{{workloads}}',
            COALESCE(
                (
                    SELECT jsonb_object_agg(key, jsonb_strip_nulls(value))
                    FROM jsonb_each(system_profile_facts -> 'workloads')
    ...
    result = session.execute(text(sql_update_hosts_batch), {"batch_size": batch_size})
...
def remove_null_values_spf_workloads_fields(session: Session, logger: Logger, config: Config, dry_run: bool = False):
    ...
    # --- Process 'hosts' table ---
    logger.info("Analyzing 'hosts' table...")
    total_hosts_to_update = _count_affected_rows(session, "hosts")
    logger.info(f"Found {total_hosts_to_update} hosts with JSON null values in workloads")

    if dry_run:
        logger.info("[DRY RUN] Would update hosts table in batches")
    else:
        total_hosts_updated = 0
        batch_num = 0

        while total_hosts_updated < total_hosts_to_update:
            batch_num += 1
            logger.info(f"Processing hosts batch {batch_num}...")

            with session_guard(session, close=False):
                rows_updated = _update_hosts_batch(session, batch_size)
                total_hosts_updated += rows_updated

            logger.info(
                f"Batch {batch_num} complete: updated {rows_updated} hosts "
                f"({total_hosts_updated}/{total_hosts_to_update} total)"
            )

            # If no rows were updated, we're done (shouldn't happen, but prevents infinite loop)
            if rows_updated == 0:
                logger.info("No more hosts to update")
                break

        logger.info(f"Hosts table complete: updated {total_hosts_updated} rows")

    # --- Process 'system_profiles_dynamic' table ---
    logger.info("Analyzing 'system_profiles_dynamic' table...")
    total_spd_to_update = _count_affected_rows(session, "system_profiles_dynamic")
