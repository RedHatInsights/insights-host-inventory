...
if [[ -z "${SONAR_PATH}" ]]
then
    echo "Scanner jar required, download from here https://binaries.sonarsource.com/Distribution/sonar-scanner-cli/sonar-scanner-cli-4.5.0.2216.zip then set the env var with SONAR_PATH=/path/to/sonar-scanner/lib/sonar-scanner-cli-4.5.0.2216.jar"
    exit 1
fi
