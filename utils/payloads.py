...
def build_rhsm_payload():
    ...
    Builds on top of build_host_chunk() for canonical fact randomization,
    then overlays RHSM-specific facts namespace and reporter.
    """
...
def build_qpc_payload():
    ...
    Builds on top of build_host_chunk() for canonical fact randomization,
    then overlays QPC-specific facts, reporter, and a minimal system profile.
    """
    payload = build_host_chunk()
    payload["reporter"] = "yupana"
    payload["facts"] = [
        {
            "namespace": "qpc",
            "facts": {
