...
def _display_name_filter(display_name: str) -> list:
    return [Host.display_name.ilike(f"%{escape_ilike_value(display_name).replace('*', '%')}%")]
...
def _stale_timestamp_per_reporter_filter(
    ...
    Expects each reporter value as an ISO 8601 string in JSONB
    (``{"reporter": "<ISO last_check_in>"}`` per key).
    ...
    if reporter.startswith("!"):
        ...
        for rep in reporter_list:
            ...
            conditions.append(reporter_not_culled)
...
def _hostname_or_id_filter(hostname_or_id: str) -> tuple:
    wildcard_id = f"%{escape_ilike_value(hostname_or_id).replace('*', '%')}%"
    filter_list = [
