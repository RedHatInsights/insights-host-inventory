...
def _stale_timestamp_per_reporter_filter(gt=None, lte=None, reporter=None, org_id=None):
    if get_flag_value(FLAG_INVENTORY_NEW_STALE_TIMESTAMP_PER_REPORTER_FILTER) and org_id is not None:
        # Use new behavior: calculate timestamps from last_check_in on-the-fly
    ...
    # Fallback to old behavior if:
    # - Feature flag is disabled, OR
    # - Feature flag is enabled but org_id is not provided
    if get_flag_value(FLAG_INVENTORY_NEW_STALE_TIMESTAMP_PER_REPORTER_FILTER) and org_id is None:
