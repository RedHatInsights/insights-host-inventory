...
Computed sort fields (virtual fields derived from multiple columns) are declared via
`__computed_sortable_fields__`, mapping field names to classmethod names that return SQL expressions.
"""
...
def _build_app_sort_field_map() -> dict[str, tuple[type[HostAppDataMixin], str]]:
    ...
    and computed sort fields via `__computed_sortable_fields__`.
    ...
    for app_name, model_class in get_app_data_models().items():
        ...
        computed_fields = getattr(model_class, "__computed_sortable_fields__", {})
...
def resolve_app_sort(order_by: str | None) -> tuple[type[HostAppDataMixin], ColumnElement] | None:
    ...
    if order_by in field_map:
        model_class, field_name = field_map[order_by]
        computed_fields = getattr(model_class, "__computed_sortable_fields__", {})
        if field_name in computed_fields:
            method_name = computed_fields[field_name]
            return model_class, getattr(model_class, method_name)()
        return model_class, getattr(model_class, field_name)
