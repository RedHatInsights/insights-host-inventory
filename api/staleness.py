...
def reset_staleness():
    """
    Reset custom staleness configuration to system defaults.

    This endpoint provides a more intuitive way to reset staleness configuration
    compared to the DELETE endpoint. It performs the same operation but uses POST
    semantics which is more discoverable and aligns with common API patterns.
    """
    identity = get_current_identity()
    request_id = threadctx.request_id
    try:
        ...
        # If no custom staleness exists, just return the defaults
        staleness = get_sys_default_staleness_api(identity)
        return flask_json_response(serialize_staleness_response(staleness), HTTPStatus.OK)
