...
def _update_hosts_staleness_async(identity, app, staleness):
    with app.app_context():
        ...
        try:
            ...
            staleness = serialize_staleness_to_dict(staleness)
            ...
            if num_hosts > 0:
                ...
                for host in hosts_query.yield_per(500):
                    ...
                    serialized_host = serialize_host(host, for_mq=False, staleness_timestamps=st, staleness=staleness)

                    # Create host update event and append it to an array
                    event, headers = _emit_host_updated_event(serialized_host, host, identity)
...
def _emit_host_updated_event(serialized_host, host, identity):
...
def _validate_flag_and_call_thread(identity, created_staleness):
    """
...
def create_staleness(body):
    ...
    try:
        ...
        _validate_flag_and_call_thread(identity, created_staleness)
...
def delete_staleness():
    ...
    try:
        remove_staleness()
        staleness = get_sys_default_staleness_api(identity)
        _validate_flag_and_call_thread(identity, staleness)
...
def update_staleness(body):
    ...
    try:
        ...
        _validate_flag_and_call_thread(identity, updated_staleness)
