...
def patch_host(host_id, host_data):
    try:
        ...
        logger.exception(
            "Input validation error while patching host: %s - %s" % (host_id, host_data)
        )
        return (
            {
                "status": 400,
    ...
    query = _get_host_list_by_id_list(current_identity.account_number, [host_id])

    host_to_update = query.first()

    if host_to_update is None:
        logger.debug("Failed to find host (id=%s) during patch operation" % (host_id))
