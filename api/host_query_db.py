...
def params_to_order_by(
    ...
    if allow_app_fields and is_app_sort_field(order_by):
        _, column = get_app_sort_model_and_column(order_by)  # type: ignore [arg-type]
...
def get_host_list_for_views(
    ...
    if is_app_sort_field(param_order_by):
        # is_app_sort_field guarantees param_order_by is not None
        app_sort_model = get_app_sort_model(param_order_by)  # type: ignore[arg-type]
