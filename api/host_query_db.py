...
def get_sparse_system_profile(
    ...
    requested_fields = fields.get("system_profile", []) if fields else []
    workloads_requested = "workloads" in requested_fields
    workloads_needed_for_compat = (
        get_flag_value(FLAG_INVENTORY_WORKLOADS_FIELDS_BACKWARD_COMPATIBILITY)
        and requested_fields
        and any(field in legacy_workload_fields for field in requested_fields)
    )
    ...
    if fields and fields.get("system_profile"):
        ...
        fields_to_fetch = list(fields["system_profile"])
            ...
            if host_data["system_profile"]:
                ...
                if requested_fields:
                    ...
                    for field in legacy_workload_fields:
                        if field in host_data["system_profile"] and field not in requested_fields:
