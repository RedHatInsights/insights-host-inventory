...
def init_cache(app_config, flask_app):
    global CACHE
    global CACHE_CONFIG
    global CACHE_SEMAPHORE
    cache_type = "NullCache"
    logger.info("Initializing Cache")

    CACHE_SEMAPHORE = threading.Semaphore(app_config.api_cache_max_thread_pool_workers)
...
def _delete_keys_redis_spawned(cache_key, wildcard=True):
    global CACHE_SEMAPHORE
    with CACHE_SEMAPHORE:
...
def delete_keys(cache_key, wildcard=True, spawn=False):
    ...
    if CACHE_CONFIG and CACHE_CONFIG.get("CACHE_TYPE") == CACHE_TYPE_REDIS_CACHE and cache_key:
        if spawn:
            thread = threading.Thread(target=_delete_keys_redis_spawned, args=(cache_key, wildcard))
            thread.start()
        else:
            _delete_keys_redis(cache_key=cache_key, wildcard=wildcard)
