...
into the new partitioned tables ('system_profiles_dynamic' and
'system_profiles_static'). It uses a descending pagination strategy based
on the 'modified_on' column for efficiency.
...
def copy_profile_data_in_batches(session: Session, logger: Logger):
    ...
    """
    main_batch_size = int(os.getenv("SP_TABLE_MIGRATION_BATCH_SIZE", 10000))
    sub_batch_size = int(os.getenv("SP_TABLE_MIGRATION_SUB_BATCH_SIZE", 500))
    last_modified_on = "9999-12-31 23:59:59+00"
    last_id = "ffffffff-ffff-ffff-ffff-ffffffffffff"
    total_rows_copied = 0

    logger.info(
        f"Starting batched data migration with a main batch size of {main_batch_size} "
        f"and a sub-batch size of {sub_batch_size}."
    )

    while True:
        fetch_start_time = time.perf_counter()
        logger.info(f"Fetching next main batch of hosts modified before: {last_modified_on} - {last_id}")
        ...
        batch_to_process = session.execute(
            text(f"""
                ...
                LIMIT :main_batch_size;
            """),
            {"last_modified_on": last_modified_on, "last_id": last_id, "main_batch_size": main_batch_size},
        ).fetchall()
        ...
        for i in range(0, len(id_list), sub_batch_size):
            sub_batch_ids = id_list[i : i + sub_batch_size]

            # --- Insert into DYNAMIC table ---
            dynamic_insert_sql = f"""
                INSERT INTO {INVENTORY_SCHEMA}.system_profiles_dynamic (
                    org_id, host_id, captured_date, running_processes, last_boot_time, installed_packages,
                ...
                SELECT
                    ...
                    (SELECT array_agg(value) FROM jsonb_array_elements_text(h.system_profile_facts -> 'running_processes')),
                    (h.system_profile_facts ->> 'last_boot_time')::timestamptz,
                    (SELECT array_agg(value) FROM jsonb_array_elements_text(h.system_profile_facts -> 'installed_packages')),
                    ...
                    (SELECT array_agg(value) FROM jsonb_array_elements_text(h.system_profile_facts -> 'kernel_modules')),
                ...
                WHERE h.id = ANY(:sub_batch_ids)
                ON CONFLICT (org_id, host_id) DO NOTHING;
            """
            session.execute(text(dynamic_insert_sql), {"sub_batch_ids": sub_batch_ids})

            # --- Insert into STATIC table ---
            static_insert_sql = f"""
                INSERT INTO {INVENTORY_SCHEMA}.system_profiles_static (
                    org_id, host_id, arch, basearch, bios_release_date, bios_vendor, bios_version, bootc_status,
                    cloud_provider, conversions, cores_per_socket, cpu_model, disk_devices, dnf_modules, enabled_services,
                    gpg_pubkeys, greenboot_fallback_detected, greenboot_status, host_type, image_builder,
                    infrastructure_type, infrastructure_vendor, insights_client_version, installed_packages_delta,
                    installed_services, intersystems, is_marketplace, katello_agent_running, number_of_cpus,
                    number_of_sockets, operating_system, os_kernel_version, os_release, owner_id, public_dns,
                    public_ipv4_addresses, releasever, rhc_client_id, rhc_config_state, rhel_ai, rhsm,
                ...
                SELECT
                    ...
                    (SELECT array_agg(value) FROM jsonb_array_elements_text(h.system_profile_facts -> 'installed_packages_delta')),
                    (SELECT array_agg(value) FROM jsonb_array_elements_text(h.system_profile_facts -> 'installed_services')),
                    ...
                    (SELECT array_agg(value) FROM jsonb_array_elements_text(h.system_profile_facts -> 'public_ipv4_addresses')),
                    ...
                    (SELECT array_agg(value) FROM jsonb_array_elements(h.system_profile_facts -> 'rpm_ostree_deployments')),
                ...
                ON CONFLICT (org_id, host_id) DO NOTHING;
            """
            session.execute(text(static_insert_sql), {"sub_batch_ids": sub_batch_ids})
        ...
        logger.info(
            f"Main batch complete in {batch_duration:.2f}s. "
