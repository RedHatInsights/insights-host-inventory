...
from lib.middleware import get_rbac_workspace_by_id_using_service_account
...
def _update_hosts_for_group_changes(host_id_list: list[str], group_id_list: list[str], identity: Identity):
    ...
    serialized_groups = [
        serialize_group(
            get_group_by_id_from_db(group_id, identity.org_id),
            identity.org_id,
...
def get_or_create_ungrouped_hosts_group_for_identity(identity: Identity) -> Group:
    ...
    if inventory_config().bypass_kessel:
        ...
        if is_rbac_v2_enabled(identity.org_id):
            group = get_rbac_workspace_by_id_using_service_account(str(workspace_id), identity.org_id)
        else:
