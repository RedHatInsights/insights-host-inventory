...
def find_stale_hosts(identity, last_run_secs):
    logger.debug("finding stale hosts with custom staleness")
    staleness_obj = serialize_staleness_to_dict(get_staleness_obj(identity=identity))
    staleness_conditions = [
        or_(
            False,
            *find_stale_host_in_window(staleness_obj, host_type, last_run_secs, stale_timestamp_filter),
        )
        for host_type in HOST_TYPES
    ]
...
def find_stale_host_sys_default_staleness(last_run_secs):
    ...
    staleness_conditions = [
        or_(
            False,
            *find_stale_host_in_window(sys_default_staleness, host_type, last_run_secs, stale_timestamp_filter),
        )
        for host_type in HOST_TYPES
    ]
