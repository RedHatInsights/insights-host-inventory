...
def add_host(
    ...
    if existing_hosts:
        ...
        matched_host = find_existing_host(identity, extract_canonical_facts_from_host(input_host), existing_hosts)
    if matched_host is None:
        # If the list of existing hosts was not provided, or the match was not found, try querying DB
        matched_host = find_existing_host(identity, extract_canonical_facts_from_host(input_host))
...
def contains_no_incorrect_facts_filter(canonical_facts: dict[str, Any]) -> BinaryExpression:
    ...
    # Incorrect value = AND( column exists/has value, NOT( column == value ) )
    # -> NOT( OR( *Incorrect values ) )
    filter_: tuple = ()
    for key, value in canonical_facts.items():
        # Get the corresponding column from the Host model
        column = getattr(Host, key, None)
        if column is None:
            continue

        # For JSONB array fields (ip_addresses, mac_addresses), use containment
        # For other fields, use direct equality
        if key in ("ip_addresses", "mac_addresses"):
            # For JSONB arrays, check if the array contains the value(s)
            if isinstance(value, list):
                # If value is a list, check if the host's array contains all elements
                # Incorrect means: column exists (is not null) AND NOT(contains value)
                filter_ += (and_(column.isnot(None), not_(column.contains(value))),)
            else:
                # Single value - check if it's in the array
                filter_ += (and_(column.isnot(None), not_(column.contains([value]))),)
        else:
            # For regular columns: column exists (is not null) AND NOT(column == value)
...
def contains_no_incorrect_facts_filter_in_memory(host: Host, canonical_facts: dict[str, Any]) -> bool:
    def _per_fact_query(host: Host, key: str, value: Any) -> bool:
        ...
        host_canonical_facts = extract_canonical_facts_from_host(host)
        if key not in host_canonical_facts:
            return True

        host_value = host_canonical_facts[key]

        # For JSONB array fields (ip_addresses, mac_addresses), use containment check
        if key in ("ip_addresses", "mac_addresses"):
            if isinstance(value, list):
                # Check if all elements in value are contained in host's array
                if not isinstance(host_value, list):
                    return False
                return all(item in host_value for item in value)
            else:
                # Single value - check if it's in the host's array
                if not isinstance(host_value, list):
                    return False
                return value in host_value
        else:
            # Direct equality for non-JSONB fields
...
def matches_at_least_one_canonical_fact_filter(canonical_facts: dict[str, Any]) -> BooleanClauseList:
    ...
    # Correct value = column matches value
    # -> OR( *correct values )
    filter_: tuple = ()
    for key, value in canonical_facts.items():
        ...
        if key in ("ip_addresses", "mac_addresses"):
            # For JSONB arrays, check if the array contains the value(s)
            # Using JSONB containment: host_array @> search_value
            if isinstance(value, list):
                # If value is a list, check if the host's array contains all elements
                filter_ += (column.contains(value),)
            else:
                # Single value - check if it's in the array
                filter_ += (column.contains([value]),)
        else:
            # Direct column comparison for non-JSONB fields
...
def matches_at_least_one_canonical_fact_filter_in_memory(host: Host, canonical_facts: dict[str, Any]) -> bool:
    ...
    for key, value in canonical_facts.items():
        # Don't include the second component of compound canonical facts
        # in the OR logic. It was already included in the AND logic generated
        # by contains_no_incorrect_facts_filter_in_memory().
        if key in COMPOUND_ID_FACTS:
            continue
        ...
        if key in ("ip_addresses", "mac_addresses"):
            if isinstance(value, list):
                ...
                if not isinstance(host_value, list):
                    continue
                if all(item in host_value for item in value):
                    return True
            else:
                # Single value - check if it's in the host's array
                if not isinstance(host_value, list):
                    continue
                if value in host_value:
                    return True
        else:
            # Direct equality for non-JSONB fields
            if host_value == value:
...
def update_system_profile(input_host: Host | LimitedHost, identity: Identity):
    ...
    if input_host.id:
        ...
        existing_host = find_existing_host(identity, extract_canonical_facts_from_host(input_host))
