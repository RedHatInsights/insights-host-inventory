...
def add_host(input_host, config, update_system_profile=True):
    """
    ...
    with db_session_guard():
        ...
        if existing_host:
            return update_existing_host(existing_host, input_host, config, update_system_profile)
        else:
            return create_new_host(input_host, config)
...
def create_new_host(input_host, config):
    logger.debug("Creating a new host")
    input_host.save()
    db.session.commit()
    metrics.create_host_count.inc()
    logger.debug("Created host:%s", input_host)
    return serialize_host(input_host, config), AddHostResults.created


@metrics.update_host_commit_processing_time.time()
def update_existing_host(existing_host, input_host, config, update_system_profile):
    ...
    return serialize_host(existing_host, config), AddHostResults.updated
